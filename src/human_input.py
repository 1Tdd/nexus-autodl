"""
src/human_input.py - Human-like mouse control using Bézier curves.
"""
...
class HumanMouse:
    """
    Mouse controller that moves in arcs (Bézier curves) with:
    - Variable speed (accelerate/decelerate)
    - Jitter (hand tremor)
    - Overshoot (miss and correct)
    - Hesitation (pause to think)
    """
    ...
    def move_to(self, x: int, y: int, duration: float = 0.0) -> None:
        """
        Move mouse to x,y using a cubic Bezier curve.
        """
        start = self._pos()
        end = Point(x, y)
